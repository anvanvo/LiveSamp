...
class RtcEventHandler : IRtcEngineEventHandler() {
    ...
    private val mEventHandlerList: ConcurrentHashMap<DuringCallEventHandler, Int> = ConcurrentHashMap<DuringCallEventHandler, Int>()

    fun addEventHandler(handler: DuringCallEventHandler) {
        mEventHandlerList[handler] = 0
    }

    fun removeEventHandler(handler: DuringCallEventHandler) {
    ...
    override fun onFirstRemoteVideoDecoded(uid: Int, width: Int, height: Int, elapsed: Int) {
        // log.debug("onFirstRemoteVideoDecoded " + (uid.toLong() and 0xFFFFFFFFL) + " " + width + " " + height + " " + elapsed)
        val it: Iterator<DuringCallEventHandler> = mEventHandlerList.keys.iterator()
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
    ...
    override fun onUserJoined(uid: Int, elapsed: Int) {
        // // log.debug("onUserJoined " + (uid.toLong() and 0xFFFFFFFFL) + elapsed)
        val it: Iterator<DuringCallEventHandler> = mEventHandlerList.keys.iterator()
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
    ...
    override fun onUserOffline(uid: Int, reason: Int) {
        // log.debug("onUserOffline " + (uid.toLong() and 0xFFFFFFFFL) + " " + reason)
    ...
    override fun onUserMuteVideo(uid: Int, muted: Boolean) {
        // log.debug("onUserMuteVideo " + (uid.toLong() and 0xFFFFFFFFL) + " " + muted)
        val it: Iterator<DuringCallEventHandler> = mEventHandlerList.keys.iterator()
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_USER_VIDEO_MUTED, uid, muted)
        }
    }
    ...
    override fun onRemoteVideoStats(stats: RemoteVideoStats) {
        ...
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_USER_VIDEO_STATS, stats)
        }
    }
    ...
    override fun onAudioVolumeIndication(speakerInfos: Array<AudioVolumeInfo?>?, totalVolume: Int) {
        ...
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_SPEAKER_STATS, (speakerInfos as Any?)!!)
        }
    }
    ...
    override fun onLastmileQuality(quality: Int) {
        ...
        while (it.hasNext()) {
            ...
            if (handler is BeforeCallEventHandler) {
                (handler as BeforeCallEventHandler).onLastmileQuality(quality)
            }
        }
    }
    ...
    override fun onLastmileProbeResult(result: LastmileProbeResult) {
        ...
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            if (handler is BeforeCallEventHandler) {
                (handler as BeforeCallEventHandler).onLastmileProbeResult(result)
            }
        }
    }
    ...
    override fun onError(error: Int) {
        ...
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_AGORA_MEDIA_ERROR, error, RtcEngine.getErrorDescription(error))
        }
    }
    ...
    override fun onStreamMessage(uid: Int, streamId: Int, data: ByteArray?) {
        // log.debug("onStreamMessage " + (uid.toLong() and 0xFFFFFFFFL) + " " + streamId + " " + Arrays.toString(data))
        val it: Iterator<DuringCallEventHandler> = mEventHandlerList.keys.iterator()
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_DATA_CHANNEL_MSG, uid, data!!)
        }
    }
    ...
    override fun onStreamMessageError(uid: Int, streamId: Int, error: Int, missed: Int, cached: Int) {
        // log.warn("onStreamMessageError " + (uid.toLong() and 0xFFFFFFFFL) + " " + streamId + " " + error + " " + missed + " " + cached)
        val it: Iterator<DuringCallEventHandler> = mEventHandlerList.keys.iterator()
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_AGORA_MEDIA_ERROR, error, "on stream msg error " + (uid.toLong() and 0xFFFFFFFFL) + " " + missed + " " + cached)
        }
    }
    ...
    override fun onConnectionLost() {
        ...
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_APP_ERROR, Constant.AppError.NO_CONNECTION_ERROR)
        }
    }
    ...
    override fun onJoinChannelSuccess(channel: String, uid: Int, elapsed: Int) {
        // log.debug("onJoinChannelSuccess " + channel + " " + (uid.toLong() and 0xFFFFFFFFL) + "(" + uid + ") " + elapsed)
        val it: Iterator<DuringCallEventHandler> = mEventHandlerList.keys.iterator()
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
    ...
    override fun onAudioRouteChanged(routing: Int) {
        ...
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_AUDIO_ROUTE_CHANGED, routing)
        }
    }
    ...
    override fun onWarning(warn: Int) {
        ...
        while (it.hasNext()) {
            val handler: DuringCallEventHandler = it.next()
            handler.onExtraCallback(Constant.AGEventHandler.EVENT_TYPE_ON_AGORA_MEDIA_ERROR, warn, msg)
        }
    }
